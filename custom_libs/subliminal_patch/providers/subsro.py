    ...
    def get_matches(self, video):
        ...
        if isinstance(video, Movie):
            ...
            matches |= guess_matches(
                video,
                guessit(
                    f"{self.title} {self.season} {self.year} {self.release_info}", {"type": "movie"}
                ),
            )
...
class SubsRoProvider(Provider, ProviderSubtitleArchiveMixin):
    ...
    def query(self, language, imdb_id, video):
        ...
        for result in soup.find_all("div", class_="md:col-span-6"):
            if (
                "flag-rom" in result.find("img")["src"] and language != Language("ron")
            ) or (
                "flag-eng" in result.find("img")["src"] and language != Language("eng")
            ):
            ...
            div_tag = result.find("div", class_="col-span-2 lg:col-span-1")
            download_link = None
            if div_tag:
    ...
    def download_subtitle(self, subtitle):
        ...
        response = self.session.get(subtitle.page_link)
